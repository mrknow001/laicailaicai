...
- `/web`：将 Web 页面、前端源码或 JS 作为前端资产入口，提取接口、参数、认证逻辑和业务线索。
- `/app`：将 App 安装包、反编译目录或 App 分析材料作为前端资产入口，提取接口、参数、认证逻辑和业务线索。
- `/mapp`：将微信小程序 id、小程序包或源码作为前端资产入口，解包或读取后提取接口、页面路由、参数和业务线索。
...
- `interface-source`：统一处理 Web / App / 小程序等前端资产作为接口来源的分析。
